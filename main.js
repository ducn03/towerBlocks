...
class Game {
  ...
  endGame() {
    ...
    // Lấy thông tin điểm cao nhất từ localStorage
    this.highScore = JSON.parse(localStorage.getItem("highScore")) || null;

    // Nếu điểm hiện tại vượt qua điểm cao nhất trước đó
    if (this.blocks.length - 2 > (this.highScore ? this.highScore.score : 0)) {
        const playerName = this.highScore ? this.highScore.name : null;
    
        // Nếu chưa có tên người chơi hoặc điểm hiện tại cao hơn
        alert(`Tuyệt vời! ${playerName || ''} đã lập một kỷ lục mới !!!`);
        const newHighScore = {
            name: playerName || prompt("Xin hãy nhập tên của bạn:"),
            score: this.blocks.length - 2,
        };
    
        // Lưu điểm mới vào localStorage và cập nhật điểm cao nhất
  ...
  updateHighScore() {
    ...
    highScoreContainer.innerHTML = `${this.highScore.name}: ${this.highScore.score}`;
  }
}
let game = new Game();
